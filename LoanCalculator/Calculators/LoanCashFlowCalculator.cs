        ...
        public List<LoanCashFlow> GenerateLoanCashFlows(Loan loan, decimal defaultMultiplier = 1.0m, decimal prepayMultiplier = 1.0m)
        {
            ...
            foreach (var payDate in payDates)
            {
                ...
                var scheduledPrincipal = lastLoanCashFlow == null ? 0.0m : 
                    (decimal) Financial.PPmt((double) (loan.CouponRate / 100.0m / 12.0m), paymentCount, loan.Term, (double) (-1.0m * loan.Invested), 0, PaymentDue.EndOfPeriod);
                var scheduledInterest = lastLoanCashFlow == null ? 0.0m : 
                    (decimal) Financial.Pmt((double) (loan.CouponRate / 100.0m / 12.0m), loan.Term, (double) (-1.0m * loan.Invested), 0, PaymentDue.EndOfPeriod) - scheduledPrincipal;
                var scheduledBalance = lastLoanCashFlow == null ? loan.Invested : (decimal) lastLoanCashFlow.ScheduledBalance - scheduledPrincipal;
                ...
                var @default = lastLoanCashFlow == null ? 0.0m : lastLoanCashFlow.Balance * lastLoanCashFlow.DefaultRate * defaultMultiplier / 100.0m;
                var servicingCF = lastLoanCashFlow == null ? 0.0m : (lastLoanCashFlow.Balance - @default) * loan.ServicingFee / 100.0m / 12.0m;
                var recovery = lastLoanCashFlow == null ? 0.0m : (decimal) loan.RecoveryRate * @default / 100.0m;
                var prepay = lastLoanCashFlow == null ? 0.0m : 
                    (lastLoanCashFlow.Balance - (lastLoanCashFlow.Balance - scheduledInterest) * scheduledPrincipal / lastLoanCashFlow.ScheduledBalance) 
                    * prepaySpeed[paymentCount] * prepayMultiplier / 100.0m;
                var principal = lastLoanCashFlow == null ? 0.0m : ((lastLoanCashFlow.Balance - @default) * scheduledPrincipal / lastLoanCashFlow.ScheduledBalance) + prepay;
                var balance = lastLoanCashFlow == null ? loan.Invested : lastLoanCashFlow.Balance - principal - @default;
                var interestAmount = lastLoanCashFlow == null ? 0.0m : (lastLoanCashFlow.Balance - @default) * loan.CouponRate / 100.0m / 12.0m;
